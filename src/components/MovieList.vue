<template>
  <div class="wrapper">
    <section class="flex justify-end pr-20 mt-5">
      ...
      <div class="sort">
        <select name="sortBy" id="select" @input="$emit('update:sortValue', $event.target.value)">
...
defineProps(['movies', 'modelValue'])
// eslint-disable-next-line no-undef
defineEmits(['update:modelValue', 'update:sortValue'])
</script>
